...
def _compile_and_deploy_contract(name):
	...
	cmd = f'starknet deploy --contract {name}_compiled.json --network=alpha-goerli'
	#print("test",cmd)
	cmd = cmd.split(' ')
...
def _poll_list_tx_hashes_until_all_accepted(list_of_tx_hashes, interval_in_sec):
	...
	while True:
		...
		for i, tx_hash in enumerate(list_of_tx_hashes):
			...
			ret = json.loads(ret)
			if ret['tx_status'] != 'ACCEPTED_ON_L1':    #'ACCEPTED_ONCHAIN'
...
def _admin_store_addresses(name, contract_addr, idx, store_addr):
	cmd = f"starknet invoke --network=alpha-goerli --address {contract_addr} --abi ../{name}_abi.json --function admin_store_addresses --inputs {idx} {store_addr}"
	cmd = cmd.split(' ')
	#print(cmd)
	ret = subprocess_run(cmd)
	ret = ret.split(': ')
...
def _inference(contract_addr, img_input):
	...
	cmd = f"starknet call --network=alpha-goerli --address {contract_addr} --abi ../mnist_abi.json --function inference --inputs {img_input_joined}"
	cmd = cmd.split(' ')
	ret = subprocess_run(cmd)
	return ret
...
def _call(name, contract_addr, func_name, img_input):
	img_input_string = [str(e) for e in img_input]
	img_input_joined = ' '.join(img_input_string)
	cmd = f"starknet call --network=alpha-goerli --address {contract_addr} --abi ../{name}_abi.json --function {func_name} --inputs {img_input_joined}"
	cmd = cmd.split(' ')
	ret = subprocess_run(cmd)
	return ret
...
def _call_compute(name):
	img_input_string = [str(e) for e in IMG_INPUT]
	img_input_joined = ' '.join(img_input_string)
	cmd = f"starknet call --network=alpha-goerli --address {deployed_extracted[name]['addr']} --abi ../{name}_abi.json --function compute --inputs {img_input_joined}"
	cmd = cmd.split(' ')
	ret = subprocess_run(cmd)
	return ret
...
# with open('deploy_log_addresses.txt', 'w') as f:
...
###############
# Create log_contract_deployment by copy pasting terminal returns
...
for sop_idx in range(32):
	print("print arguments:",'mnist_v1', deployed_extracted['mnist_v1']['addr'], sop_idx, deployed_extracted[f'mnist_v1_sop{sop_idx}']['addr'])
